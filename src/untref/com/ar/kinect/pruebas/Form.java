...
public class Form extends JFrame implements ActionListener {
	...
	private JLabel label_scrollBarBordesCannyUmbralBajo;
	private JLabel label_scrollBarBordesCannyUmbralAlto;
	private JLabel label_Nota;
	private JRadioButton radioAlturaSupPlana;
	private JRadioButton radioAlturaSupCurva;
	private JRadioButton radioHougHSi;
	private JRadioButton radioHougHNo;
	...
	private JScrollBar scrollBarBordesCannyUmbralBajo;
	private JScrollBar scrollBarBordesCannyUmbralAlto;
	private float alpha;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == menuVistas.getItem(6)) {
			...
			setVisibilidadFiltroHough(true);
		}

		if (e.getSource() == menuVistas.getItem(7)) {
			this.setVisibilidadScrollBarCanny(true);
			setVisibilidadFiltroHough(true);
		...
		panelDeOpciones.setBorder(BorderFactory.createTitledBorder(border, title, TitledBorder.CENTER, TitledBorder.TOP,
				null, Color.BLUE));
		panelDeOpcionesBordes.setBorder(BorderFactory.createTitledBorder(border, title, TitledBorder.CENTER,
	...
	private void opcionesDeExportarDeDatos() {
		...
		label_Nota = builder.construirLabel(
				"<html>(*)De no ingresar una ruta para el archivo<br> el mismo se guardará en el directorio <br>donde está la aplicación.</html>",
				0, 4);
	...
	private void radioButtonBordes(int posX, int posY, BasicComponentBuilder builder) {

		radioHougHSi = builder.construirRadioButton("Activar Hough", posX, posY);
		radioHougHNo = builder.construirRadioButton("Desactivar Hough", posX, posY + 1);
		radioHougHNo.setSelected(true);
		ButtonGroup radioButtonsHough = new ButtonGroup();
		radioButtonsHough.add(radioHougHSi);
	...
	private void setVisibilidadScrollBarCanny(boolean bool) {
		label_scrollBarBordesCannyUmbralBajo.setVisible(bool);
		scrollBarBordesCannyUmbralBajo.setVisible(bool);

		label_scrollBarBordesCannyUmbralAlto.setVisible(bool);
		scrollBarBordesCannyUmbralAlto.setVisible(bool);
	}
	...
	private void setVisibilidadFiltroHough(boolean bool) {

		radioHougHSi.setVisible(bool);
		radioHougHNo.setVisible(bool);
	}
	...
	public void actualizar() {
		...
		if (bordesSeleccionadoSobel) {
			...
			if (radioHougHSi.isSelected())
				hough = true;
			imagen = data.getImagenBordesSobel(hough);
			panelDeOpciones.setBorder(null);
		...
		if (bordesSeleccionadoCanny) {
			...
			this.setVisibilidadScrollBarCanny(false);
		}
